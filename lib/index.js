...
export function apply(ctx) {
  ...
  function fire(entry) {
    ...
    const deliverToParent = () => {
      ...
      if (liveParent !== undefined) {
        console.log(`timer-scheduler: subagent ${sessionId} not live; delivering reminder to parent ${parentSession}`)
        deliver(liveParent)
        return true
      }
      void (async () => {
        let handle = resumed.get(parentSession)
        if (handle === undefined) {
          handle = await ctx.agents.resume({ resumeSessionId: parentSession })
          resumed.set(parentSession, handle)
          console.log(`timer-scheduler: cold-resumed parent ${parentSession} for subagent reminder`)
        }
        deliver(handle.agent)
      })().catch((err) => {
        console.warn(`timer-scheduler: parent ${parentSession} cold resume failed; reminder skipped: ${String((err && err.message) || err)}`)
      })
      return true
    }
  ...
  ctx.on('mission/task-settled', ({ taskId, sessionId }) => {
    ...
    for (const entry of pending.values()) {
      ...
      const matchesTask = entry.subject === taskId || (entry.note && entry.note.includes(taskId))
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      const parentSession = header?.origin === 'subagent' ? header.parentSession : undefined
