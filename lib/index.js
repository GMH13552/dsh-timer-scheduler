...
export function apply(ctx) {
  ...
  function persist() {
    ...
    for (const entry of pending.values()) {
      list.push({ id: entry.id, note: entry.note, dueMs: entry.dueMs, sessionId: entry.sessionId, ...(entry.subject ? { subject: entry.subject } : {}) })
    }
    saveReminders(list)
  }
  ...
  function fire(entry) {
    ...
    // Cold resume: load the persisted session and wake it with the reminder.
    void (async () => {
      ...
      console.warn(`timer-scheduler: cold resume failed for ${sessionId}; reminder skipped: ${String((err && err.message) || err)}`)
    })
  }
  ...
  function schedule(note, dueMs, sessionId, subject) {
    const entry = { id: makeId(), note, dueMs, sessionId, subject: subject || undefined, cancel: null }
  ...
  ctx.on('agent/inbox/inserted', ({ agent, message }) => {
    ...
    let childId = typeof source.senderSessionId === 'string' ? source.senderSessionId : ''
    if (childId === '') {
    ...
    for (const entry of pending.values()) {
      ...
      if (entry.subject === childId) {
        toCancel.push(entry.id)
      } else if (!entry.subject && entry.note && entry.note.includes(childId)) {
        toCancel.push(entry.id)
      }
    }
    if (toCancel.length > 0) {
      console.log(`timer-scheduler: subagent ${childId} settled; canceling ${toCancel.length} fallback reminder(s) for ${agent.id}`)
  ...
  for (const r of loadReminders()) {
    const entry = { id: r.id, note: r.note, dueMs: r.dueMs, sessionId: r.sessionId, subject: typeof r.subject === 'string' ? r.subject : undefined, cancel: null }
    pending.set(entry.id, entry)
    arm(entry)
  }
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      const entry = schedule(String(args.note), dueMs, agent.id, typeof args.subject === 'string' ? args.subject : undefined)
