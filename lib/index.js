  ...
  function fire(entry) {
    ...
    const deliverToParent = () => {
      ...
      if (liveParent !== undefined) {
        console.log(`timer-scheduler: session ${sessionId} not live; delivering reminder to parent ${parentSession}`)
        deliver(liveParent)
        return true
      }
      void (async () => {
        ...
        if (handle === undefined) {
          ...
          console.log(`timer-scheduler: cold-resumed parent ${parentSession} for child-session reminder`)
        }
